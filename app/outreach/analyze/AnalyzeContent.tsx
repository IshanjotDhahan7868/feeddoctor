...
export default function AnalyzeContent() {
  ...
  const [issues, setIssues] = useState<Record<string, ProductIssue[]> | null>(null);
  ...
  return (
    <main className="p-6 max-w-4xl mx-auto">
      ...
      {storeIds.length === 0 ? (
        <p className="text-gray-600">Please select stores from the Discover page to analyze.</p>
      ) : loading ? (
                    ...
                    <tr key={p.id} className="border-b">
                      ...
                      <td classname="px-2 py-1">
                        <ul className="list-disc list-inside text-xs text-gray-700">
                          {p.issues.map((i, idx) => <li key={idx}>{i}</li>)}
                        </ul>
                      </td>
                      <td className="px-2 py-1">
                        <ul className="list-disc list-inside text-xs text-gray-700">
                          {p.suggestions.map((s, idx) => <li key={idx}>{s}</li>)}
                        </ul>
                      </td>
                    </tr>
                  ))}
